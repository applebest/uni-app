...
function src_default(options = {}) {
  ...
  const storePath = _path.resolve.call(void 0, process.cwd(), storeDir);
  const outputDir = outputFile.replace(/(\/[^/]*).ts/, "");
  _fs.promises.readdir(outputDir).catch(() => _fs.promises.mkdir(outputDir));
  async function generateConfigFiles() {
    const storesPath = await _fs.promises.readdir(storePath);
...
`;
    _fs.promises.writeFile(outputFile, ctx, "utf-8");
  }
  generateConfigFiles();
  if (process.env.NODE_ENV === "development" || isDev) {
    const watcher = _chokidar2.default.watch(storePath);
